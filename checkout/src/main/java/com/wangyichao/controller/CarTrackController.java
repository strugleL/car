...
//        response.setContentType("text/html;charset=UTF-8");
    ...
    /**
     * 通过查找起始日期和车牌号获取逃费稽核展现结果数据
     * @param response
     * 新增！！！！！！！！！！！！！！！！！！！
     *
     * */
    @RequestMapping(value = "getDodgeTollAuditInfoByTimeAndcarNumber",method = RequestMethod.GET)
    @ResponseBody
    public void getDodgeTollAuditInfoByTimeAndcarNumber(HttpServletRequest request,HttpServletResponse response,
                                                        @RequestParam(value = "start",required = true) String startTime,
                                                        @RequestParam(value = "end",required = true) String endTime,
                                                        @RequestParam(value = "carNumber", required = true) String carNumber){
        JSONArray carInfoArr = dodgeTollAudit.getDodgeTollAuditInfoByTimeAndcarNumber(startTime,endTime,carNumber);
        response.setContentType("text/html;charset=utf-8");
        PrintWriter pw = null;
        try {
            pw = response.getWriter();
    ...
    public void updateSelect(HttpServletRequest request,HttpServletResponse response,
                             @RequestParam(value = "passCardDodge",required = true) String passCardDodge,
                             @RequestParam(value = "weightDodge",required = true) String weightDodge,
                             @RequestParam(value = "laneexSerialNo",required = true) String laneexSerialNo){
        int result = dodgeTollAudit.update(passCardDodge,weightDodge,laneexSerialNo);
        PrintWriter pw = null;
        try {
            pw = response.getWriter();
            pw.print(result);
...
//        response.setContentType("text/html;charset=UTF-8");
    ...
    public void dodgeTollCarConfirm(HttpServletRequest request, HttpServletResponse response,
        ...
        Boolean result = dodgeTollAudit.dodgeTollCarConfirm(carNo, operation,operator);
        response.setContentType("text/html;charset=utf-8");
        PrintWriter pw = null;
        try {
            pw = response.getWriter();
