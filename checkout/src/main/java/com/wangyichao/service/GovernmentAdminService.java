...
//        double dDistance = dRad * EARTH_RADIUS;
    ...
    public JSONArray getCarInformByCarNumber(String carNumber) {
        return governmentAdminDao.getCarInformByCarNumber(carNumber);
    }

    //判断当前密码是否正确
    public int isOldpw(String admin_id, String nowpw) {

        int result = governmentAdminDao.isOldpw(admin_id, nowpw);
        return result;
    }

    //修改密码
    public boolean changePw(String admin_id, String newpw) {
        boolean result = governmentAdminDao.changePw(admin_id,newpw);
        return result;
    }
