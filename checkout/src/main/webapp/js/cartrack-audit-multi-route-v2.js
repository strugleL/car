...
app.controller('carTrackAuditCtrl', function ($scope, $http, $interval) {
    ...
    var carSelected = [];
    $scope.carInfoSelected = [];
    var carExRecodeNos = [];
    $scope.isChecked = function ($event, carNo, exRecordNo) {

        var checkbox = $event.target;
        if (checkbox.checked) {
            carSelected.push(carNo);
            carExRecodeNos.push(exRecordNo);
            // alert(carNo + " is selected")
        } else {
            var carIndex = carSelected.indexOf(carNo);
            var carInfoIndex = carExRecodeNos.indexOf(exRecordNo);
            carSelected.splice(carIndex, 1);
            carExRecodeNos.splice(carInfoIndex, 1);
        ...
        // getCarLocation(carNos);
        console.log('carExRecodeNos: ' + carExRecodeNos);

        searchCarInfos(carExRecodeNos);
    ...
    $scope.infoMaps = [];
    var searchCarInfos = function (exRecordNos) {
        $scope.infoMaps = [];
        // $scope.infoMaps = [];
        if (exRecordNos.length > 0) {
            processNext(exRecordNos.slice(0));
        }
        ...
        function processNext(copiedData) {
            ...
            var infoMap = {};
            var url = rootUrl + 'getDodgeTollCarInfo?carNo=';
            $http.get(url + copiedData[0]).success(function (response) {
                if (response.length == 0) {
                    alert('没有此车辆信息');
                }
                console.log('车辆信息: ' + response);
                infoMap.carInfo = response;
                var carSearchUrl = rootUrl + 'search?carNo=';
                console.log('search location of: ' + response.carNo);
                $http.get(carSearchUrl + response.carNo).success(function (res) {

                    if (res['pathInfo'].length == 0) {
                        alert("对不起,没有对应的车辆信息");
                    } else {
                        // 渲染数据
                        var pathInfo = res['pathInfo'];
                        var lastIndex = pathInfo.length - 1;
                        ...
                        geoc.getLocation(point1, function (rs) {
                            var addComp = rs.addressComponents;
                            var startLocation = addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber;
                            setStart(startLocation);

                        });
                        geoc.getLocation(point2, function (rs) {
                            var addComp = rs.addressComponents;
                            var endLocation = addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber;
                            setEnd(endLocation);
                        });

                    }
                    // console.log('infoMap carInfo: ' + infoMap.carInfo);
                    // console.log('infoMap location: ' + infoMap.startLocation);


                    function setStart(startLocation) {
                        infoMap.startLocation = startLocation;
                    }
                    function setEnd(endLocation) {
                        infoMap.endLocation = endLocation;
                    }
                ...
                $scope.infoMaps.push(infoMap);

            }).error(function (data) {
                console.log(data);
            });
            processNext(copiedData.slice(1));
            }
        }
        // for (var i in exRecordNos) {
        //     (function () {
        //         console.log('exRecordNos[i]: '+ exRecordNos[i]);
        //         var infoMap = {};
        //         var url = rootUrl + 'getDodgeTollCarInfo?carNo=';
        //         $http.get(url + exRecordNos[i]).success(function (response) {
        //             if (response.length == 0) {
        //                 alert('没有此车辆信息');
        //             }
        //             console.log('车辆信息: ' + response);
